//! Terms as application trees with inlined atoms.

use crate::{AppSelf as App, Token, Loop, Error, Result};
...
impl<S: Into<C> + Into<V>, C, V> State<S, C, V> {
        ...
        I: Iterator<Item = Token<S>>,
    {
        match iter.next() {
            ...
            Some(Token::LPar) => {
                let app = Some(App::new(Term1::Const(s1.into())));
            ...
            Some(other) => match App::new(Term1::Const(s1.into())).reduce(ctx) {
        ...
        while let Some(tok) = token.take() {
            match tok {
                Token::Ident(s) => match iter.next() {
                    None => app.1.push(App::new(Term1::Const(s.into()))),
                    Some(other) => {
                        app.1.push(App::new(Term1::Const(s.into())));
                        token = Some(other);
                        continue;
                    }
                },
