...
//! The heads of applications are boxed.

use crate::{Abst, Token, Error, Result};
use alloc::{boxed::Box, vec::Vec};
use core::fmt::{self, Display};

type Comb<C, V> = Abst<V, App<Term<C, V>>>;

type App<Tm> = crate::App<Box<Tm>, Tm>;
...
impl<C, V> LPar<C, V> {
    fn close(mut self, mut comb: Comb<C, V>) -> Comb<C, V> {
        match self.app {
            ...
            /*
            Some(mut app) if comb.bnd.is_empty() => {
                app.1.push(*comb.app.0);
                app.1.append(&mut comb.app.1);
                Comb { bnd: self.bnd, app }
            }
            */
            Some(mut app) => {
                app.1.push(Term::Comb(comb));
                Comb { bnd: self.bnd, app }
            }
        }
    }
        ...
        I: Iterator<Item = Token<S>>,
    {
        match iter.next() {
            ...
            Some(Token::Ident(s2)) => {
                let comb = Comb {
                    bnd: bnds,
                    app: App::new(Term::Const(s1.into()).into()),
                };
            ...
            Some(Token::LPar) => {
                let app = Some(App::new(Term::Const(s1.into()).into()));
                ctx.stack.push(LPar { bnd: bnds, app });
                bnds = Vec::new();
            }
            Some(Token::RPar) => {
                let comb = Comb::new(bnds, App::new(Term::Const(s1.into()).into()));
        ...
        while let Some(tok) = token.take() {
            match tok {
                Token::Ident(s) => match iter.next() {
                    None => comb.app.1.push(Term::Const(s.into())),
                    Some(other) => {
                        comb.app.1.push(Term::Const(s.into()));
                        token = Some(other);
                        continue;
                    }
                },
                ...
                Token::RPar => match ctx.stack.pop() {
                    ...
                    None => return Ok(Loop::Return(State::Term(Term::Comb(comb), Token::RPar))),
                },
                tok => {
                    if ctx.stack.is_empty() {
                        return Ok(Loop::Return(State::Term(Term::Comb(comb), tok.map(|_| ()))));
                    } else {
                        return Err(Error::UnclosedLPar);
                    }
                }
